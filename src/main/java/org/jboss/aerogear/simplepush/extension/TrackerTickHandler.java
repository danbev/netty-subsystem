...
class TrackerTickHandler extends AbstractWriteAttributeHandler<Void> {
    ...
    protected boolean applyUpdateToRuntime(OperationContext context, ModelNode operation, String attributeName,
        ...
        if (attributeName.equals(SimplePushExtension.TICK)) {
            final String suffix = PathAddress.pathAddress(operation.get(ModelDescriptionConstants.ADDRESS)).getLastElement().getValue();
            SimplePushService service = (SimplePushService) context.getServiceRegistry(true).getRequiredService(SimplePushService.createServiceName(suffix)).getValue();
