...
class SimplePushPortHandler extends AbstractWriteAttributeHandler<Void> {

    public static final SimplePushPortHandler INSTANCE = new SimplePushPortHandler();

    private SimplePushPortHandler() {
    ...
    protected boolean applyUpdateToRuntime(OperationContext context, ModelNode operation, String attributeName,
        ...
        if (attributeName.equals(SimplePushExtension.PORT)) {
            final String suffix = PathAddress.pathAddress(operation.get(ModelDescriptionConstants.ADDRESS)).getLastElement().getValue();
            final SimplePushService service = (SimplePushService) context.getServiceRegistry(true).getRequiredService(SimplePushService.createServiceName(suffix)).getValue();
